...
class Game extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            lifes: 94,
            items: {
                item1: {bottom: 20, left: 1500, touched: false, background: `url('${inimigo1}')`},
                item2: {bottom: 30, left: 2500, touched: false, background: `url('${inimigo2}')`},
                item3: {bottom: 20, left: 3000, touched: false, background: `url('${inimigo1}')`},
                item4: {bottom: 30, left: 4000, touched: false, background: `url('${inimigo2}')`},
                item5: {bottom: 20, left: 4500, touched: false, background: `url('${inimigo1}')`},
                item6: {bottom: 30, left: 5000, touched: false, background: `url('${inimigo2}')`},
            }
        };
    ...
    animateElements() {
        ...
        // Aumenta a velociade
        this.setState({gameSpeed: (this.state.distance / 100) + 9 });

        //se esta jogando
        if (this.state.isPlaying === true) {

            //se bgOffSet menor ou igual a 900
            if (this.state.bgOffset >= 1250 ) {
        ...
        return (
            <div className="l-game-wrapper">
                ...
                <div className="c-data">
                    <h4 style={{margin: '5px 0',color: 'white',fontSize: '4vh'}}>Pontuação: {this.state.score.toFixed(0)}</h4>
